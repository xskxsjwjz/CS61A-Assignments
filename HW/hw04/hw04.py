def shuffle(s):
    ...
    "*** YOUR CODE HERE ***"
    half_len = len(s) // 2
    return [s[i // 2 + (i % 2) * half_len] for i in range(len(s))]
...
def balanced(m):
    ...
    left_arm = left(m)
    right_arm = right(m)
    return (length(left_arm) * total_mass(end(left_arm)) == length(right_arm) * total_mass(end(right_arm)) and 
...
def berry_finder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    return label(t) == 'berry' or any(berry_finder(branch) for branch in branches(t))
