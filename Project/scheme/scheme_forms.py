...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if sub_expressions is nil:
                return test if clause.first != 'else' else True
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        formal, value = current.first, scheme_eval(current.rest.first, env)
        names, vals = Pair(formal, names), Pair(value, vals)
