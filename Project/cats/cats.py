...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    tmp = [paragraph for paragraph in paragraphs if select(paragraph)]
    return tmp[k] if k < len(tmp) else ''
    # END PROBLEM 1
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    total_len = min(len(typed_words), len(source_words))
    correct = 0
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if immutable_args in cache and limit <= cache[immutable_args][1]:
            return cache[immutable_args][0]
        else:
            value = diff_function(typed, source, limit)
            cache[immutable_args] = (value, limit)
            return value
        # END PROBLEM EC
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if min_diff > word_diff and word_diff <= limit:
            min_diff = word_diff
            return_word = word
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if typed == source or limit < 0 or (typed != source and limit == 0): # Base cases should go here, you may add more base cases as needed.
        return 0
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] != source[i]:
            break
        correct += 1
    correct_ratio = correct / len(source)
    upload(dict(id=user_id, progress=correct_ratio))
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        tmp = [get_time(times, player_num, i) for player_num in player_indices]
        res[tmp.index(min(tmp))].append(words[i])
    return res
    # END PROBLEM 10
